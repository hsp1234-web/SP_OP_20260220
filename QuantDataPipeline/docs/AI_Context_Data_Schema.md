...
data/
 ├── 2024/
 │    ├── TaiwanFuturesTick/     # 台指期貨逐筆資料
 │    │    └── TX_2024-01-02.parquet
 │    ├── TaiwanOptionTick/      # 台指選擇權逐筆資料
 │    │    └── TXO_2024-01-02.parquet
 │    └── GreeksFeatures/        # 對齊並計算 IV/Greeks 後的特徵庫 (ML ready)
 │         └── TXO_Greeks_2024-01-02.parquet
 └── 2023/ ... (依此類推)
```
...
2. **無風險利率 (r)**: 程式內部定價模組預設常數使用 `r = 0.015` (1.5%)。
3. **結算日邊界值**: 若計算到期日遇到 `T <= 0` 的情況，內部引擎會防呆並自動賦予極小值 (0.0001) 避免除零錯誤。
